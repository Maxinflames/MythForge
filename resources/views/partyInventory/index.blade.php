<title>MythForge - Party Inventory</title>
@extends('layouts.app')
            ...
            @if (Session::get('user_id') == $campaign->user_id)
                <div class="center ">
                    <a class="btn btn-primary"
                        href="/campaign/{{ $campaign->campaign_id }}/party-inventory/{{ $party_inventory->party_inventory_id }}/add">Add
                        Items</a>
                    <a class="btn btn-primary"
                        href="/campaign/{{ $campaign->campaign_id }}/party-inventory/{{ $party_inventory->party_inventory_id }}/loot-group">Add
                        Loot Group</a>
                    <br />
                </div>
            @endif
                ...
                <table class="table table-striped">
                    ...
                    @foreach ($items as $item)
                        <tr>
                            ...
                            <td class="text-center">{{ $item->party_inventory_item_quantity }}</td>
                            <td>
                                <form method="POST"
                                    action="/campaign/{{ $campaign->campaign_id }}/party-inventory/{{ $party_inventory->party_inventory_id }}/send">
                                    ...
                                    <div class="inline-float width-135px">
                                        <select name="player_inventory_id" id="player_inventory_id" class="form-control">
                                            @foreach ($players as $player)
                                                <option value="{{ $player->player_inventory_id }}">
